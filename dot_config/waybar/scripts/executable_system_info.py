...
def get_gpu_info():
    ...
    # Try AMD Sysfs (drm) - Most reliable for modern AMD GPUs
    try:
        drm_path = "/sys/class/drm"
        if os.path.exists(drm_path):
            for item in os.listdir(drm_path):
                if item.startswith("card") and os.path.isdir(
                    os.path.join(drm_path, item)
                ):
                    dev_path = os.path.join(drm_path, item, "device")
                    busy_path = os.path.join(dev_path, "gpu_busy_percent")

                    if os.path.exists(busy_path):
                        # Found an active GPU sysfs node
                        with open(busy_path, "r") as f:
                            gpu_usage = int(f.read().strip())

                        # Get VRAM
                        vram_used_path = os.path.join(dev_path, "mem_info_vram_used")
                        vram_total_path = os.path.join(dev_path, "mem_info_vram_total")
                        if os.path.exists(vram_used_path) and os.path.exists(
                            vram_total_path
                        ):
                            with open(vram_used_path, "r") as f:
                                gpu_mem_used = int(f.read().strip())
                            with open(vram_total_path, "r") as f:
                                gpu_mem_total = int(f.read().strip())
                        ...
                        try:
                            ...
                            matches = re.findall(r"\[(.*?)\]", lspci_out)
                            if len(matches) >= 3:
                                gpu_name = matches[-1]
                            elif "Radeon" in lspci_out:
                                gpu_name = "AMD Radeon"
                        except Exception:
                            pass
